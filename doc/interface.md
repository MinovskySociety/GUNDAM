...
|edge_label_type|string edge_label_type()|返回EdgeLabelType的字符串，若类型是整型(int,long long,unsigned int等等)，返回"int",若类型是浮点类型(float,double,long double),返回"double"，若是std::string类型，返回"string",其他类型返回"unknown type"|
