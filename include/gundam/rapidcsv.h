...
struct LabelParams {
   ...
   * gives access to all rows as document data.
   ...
   * access to all columns as document data.
   */
...
struct SeparatorParams {
   ...
   * trailing spaces from cells read.
...
class Document {
 public:
  ...
  size_t GetColumnCount() const {
    return (mData.size() > 0)
               ? (mData.at(0).size() - (mLabelParams.mRowNameIdx + 1))
               : 0;
  }
