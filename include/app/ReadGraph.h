...
ReturnType ReadCSVGraph(GraphType<configures...>& graph,
    char *v_file, char *e_file){
    ...
    std::vector<VertexIDType> node_id =
        node_file.GetColumn<VertexIDType>(0);
    ...
    for (int i = 0; i < sz; i++){
       // std::cout<<from_id[i]<<" "<<to_id[i]<<" "<<edge_label[i]<<std::endl;
        graph.AddEdge(from_id[i], to_id[i],
